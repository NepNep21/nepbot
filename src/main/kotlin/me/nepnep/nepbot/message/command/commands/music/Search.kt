...
class Search : AbstractCommand(
    "search",
    Category.MUSIC,
    "Searches youtube for something to play"
) {
    override suspend fun execute(args: List<String>, event: MessageReceivedEvent, channel: GuildMessageChannel) {
        ...
        MusicManager.audioPlayerManager.loadItem(
            "ytsearch:${args.subList(0, args.size).joinToString(" ")}",
            object : AudioLoadResultHandler {
                override fun trackLoaded(track: AudioTrack) {
                    // Should never happen?
                    throw IllegalStateException("Single tracked loaded by ;search")
                }

                override fun playlistLoaded(playlist: AudioPlaylist) {
                    scheduler.queue(playlist.tracks[0])
                }
